    ...
    <section>
      <div className="space-y-1">
        {sortedPosts.map((post) => (
          <a
            className="flex h-9 min-h-0 items-center rounded-md px-3 py-2 transition-colors hover:bg-accent"
            href={`/writings/posts/${post.slug}`}
            key={post.slug}
            rel="noopener noreferrer"
          >
            <div className="w-full truncate text-base">{post.data.title}</div>
